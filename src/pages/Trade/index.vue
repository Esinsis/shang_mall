<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(address, index) in addrs" :key="address.id">
        <span class="username" :class="{ selected: address.isDefault == 1 }">
          {{ address.consignee }}
        </span>
        <p @click="changeDefault(address, addrs)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="(order, index) in orderList" :key="order.skuId">
          <li>
...
export default {
  ...
  methods: {
    changeDefault(address, addrs) {
      addrs.forEach(item => item.isDefault = '0');
      address.isDefault = '1';
    },
    async submitOrder() {
      let params = {
      ...
      let result = await this.$API.submitOrder(params, this.orderInfo.tradeNo);
      if (result.code == 200) {
  ...
  computed: {
    ...mapState({
      addrs: state => state.trade.addrs,
      orderInfo: state => state.trade.orderInfo
    }),
    defaultUser() {
      return this.addrs.find(item => item.isDefault == '1') || {};
    },
    orderList() {
